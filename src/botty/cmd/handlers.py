...
from .utils import run_command, escape_markdown, escape_markdown_code
...
@authorized_only
async def emby_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    service_status = await run_command("systemctl status emby-server.service --no-pager -n 0")
    ...
    message = f"*Emby Media Server Status*\n\n"
...
@authorized_only
async def adguard_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Checks the status of AdGuard Home."""
    # -n 0 suppresses logs
    service_status = await run_command("systemctl status AdGuardHome.service --no-pager -n 0")
    
    message = f"*AdGuard Home Status*\n\n```\n{escape_markdown_code(service_status)}\n```"

    await update.message.reply_text(message, parse_mode="MarkdownV2")


@authorized_only
async def network_tests_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if device_data and device_data.get("Available"):
                ...
                l1 = device_data.get('Load1')
                l5 = device_data.get('Load5')
                l15 = device_data.get('Load15')
                ...
                load_str = f"{l1:.2f}, {l5:.2f}, {l15:.2f}" if all(isinstance(l, (int, float)) for l in [l1, l5, l15]) else "N/A"
        ...
        await update.message.reply_text(f"Could not connect to the GoHome API: {escape_markdown(str(e))}", parse_mode="MarkdownV2")
